...
public final class Utils {
	...
	byte paddingByte) throws java.io.UnsupportedEncodingException {
		byte dst_array[] = new byte[ length];
		byte src_array[] = text.getBytes(characterSetName);
		if( src_array.length >= length) {
			System.arraycopy(src_array, 0, dst_array, 0, length);
		}
		else {
		
			Arrays.fill( dst_array, paddingByte);
	
			switch( alignment) {
	
				case ALIGN_LEFT:
					System.arraycopy(src_array, 0, dst_array, 0, src_array.length);
					break;
	
				case ALIGN_RIGHT:
					int t_offset = length - text.length();
					System.arraycopy(src_array, 0, dst_array, t_offset, src_array.length);
					break;
				}	
		}
		return dst_array;
